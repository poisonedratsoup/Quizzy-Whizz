...
def upload_pdf():
        ...
        if filename.endswith(".pdf"):
            with pdfplumber.open(file) as pdf:
                ...
                text = "\n".join(
                    [
                        page.extract_text()
                        for page in pdf.pages[start_pg:actual_end]
    ...
    if response.status_code != 200:
        ...
        return (
            jsonify(
                {
                    "error": "The AI is currently overwhelmed. Please wait a few seconds and try again!"
                }
            ),
            503,
        )
    ...
    try:
        ...
        return (
            jsonify({"error": "The AI created a broken structure. Please try again."}),
            500,
        )
